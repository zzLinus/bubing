...
#define printf(...) usart1_printf(__VA_ARGS__);
 ...
 * @retval         none
 */
static void imu_cali_slove(fp32 gyro[3], fp32 accel[3], fp32 mag[3],
...
static const float timing_time =
    0.001f; // tast run time , unit s.任务运行的时间 单位 s

// 加速度计低通滤波
static fp32 accel_fliter_1[3] = {0.0f, 0.0f, 0.0f};
static fp32 accel_fliter_2[3] = {0.0f, 0.0f, 0.0f};
static fp32 accel_fliter_3[3] = {0.0f, 0.0f, 0.0f};
static const fp32 fliter_num[3] = {1.929454039488895f, -0.93178349823448126f,
...
void QuaternionToEulerAngles(double qx, double qy, double qz, double qw) {
  double a, b, c; // 俯仰角pitch,滚动角roll,航向角yaw
...
void INS_task(void const *pvParameters) {
  ...
  printf("Calibrating BNO080. Pull up FS-i6 SWC to end calibration and save to "
         "flash\n");
  printf("Output in form x, y, in uTesla\n\n");
	// FIXME:
  ...
  while (1) {
    ...
    while (ulTaskNotifyTake(pdTRUE, portMAX_DELAY) != pdPASS) {
		  ...
		  double quatI = (float)BNO080_getQuatI();
      double quatJ = (float)BNO080_getQuatJ();
      double quatK = (float)BNO080_getQuatK();
      double quatReal = (float)BNO080_getQuatReal();
    ...
    /************************************/
    /**usart1_printf("gyro angle %f %f %f \n",Angle[0], Angle[1], (Angle[2]);*/
...
/**
 * @brief          get the rotation speed, 0:x-axis, 1:y-axis, 2:roll-axis,unit
 * rad/s
 * @param[in]      none
 ...
 * @retval         INS_gyro的指针
 */
extern const fp32 *get_gyro_data_point(void) { return INS_gyro; }
/**
 ...
 * @retval         INS_accel的指针
 */
extern const fp32 *get_accel_data_point(void) { return INS_accel; }
/**
...
static void imu_cmd_spi_dma(void) {
  ...
  if ((gyro_update_flag & (1 << IMU_DR_SHFITS)) &&
      !LL_DMA_IsEnabledStream(DMA2, LL_DMA_STREAM_2) &&
      !LL_DMA_IsEnabledStream(DMA2, LL_DMA_STREAM_3) &&
    ...
    SPI1_DMA_enable((uint32_t)gyro_dma_tx_buf, (uint32_t)gyro_dma_rx_buf,
                    SPI_DMA_GYRO_LENGHT);
		BNO080_dataIN(gyro_dma_rx_buf, gyro_dma_rx_buf + 4);
